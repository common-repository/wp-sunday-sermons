 ...
 * @author  WP Sunday
 * @license GPL-2.0+
 * @link    http://www.wpsunday.com/plugins/wp-sunday-sermons/
 */

/**
 * WP Sunday Featured Sermon widget class.
 *
 * @since 0.9.0
 *
 * @package WP Sunday\Widgets
 */
class WP_Sunday_Featured_Sermons extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults = array(
			...
			'content_limit'           => 120,
			'more_text'               => __( 'Listen&nbsp;Now&nbsp;&rsaquo;', 'wp-sunday' ),
		);

		$widget_ops = array(
			'classname'   => 'featured-content featured-sermon',
			'description' => __( 'Displays featured sermons with thumbnails', 'wp-sunday' ),
		);

		$control_ops = array(
			'id_base' => 'featured-sermon',
		);

		parent::__construct( 'featured-sermon', __( 'WP Sunday - Sermons', 'wp-sunday' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['posts_cat'] ) )
			$query_args['tax_query'] = array(
        		array(
          			'taxonomy'  => 'wp_sunday_sermon_series',
          			'field'     => 'term_id',
		...
		if ( have_posts() ) : while ( have_posts() ) : the_post();
			...
			if ( $instance['show_title'] )
				...
				if ( ! empty( $instance['show_title'] ) ) {
					...
					/**
					 * Filter the featured sermon widget title.
					 *
					 * @since  0.9.0
					 *
					 * @param string $title    Featured sermon title.
					 * @param array  $instance {
					 *     Widget settings for this instance.
					 *
					 *     @type string $title                   Widget title.
					 *     @type int    $posts_cat               ID of the post category.
					 *     @type int    $posts_num               Number of sermons to show.
					 ...
					 *     @type bool   $show_title              True if featured sermon title should
					 *                                           be shown, false otherwise.
					 *     @type bool   $show_content            True if featured sermon content
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'posts_cat' ); ?>"><?php _e( 'Sermon Series', 'wp-sunday' ); ?>:</label>
			<?php
			$categories_args = array(
				...
				'orderby'			=> 'Name',
				'hierarchical'		=> 1,
				'show_option_all'	=> __( 'All Series', 'wp-sunday' ),
				'hide_empty'		=> 1,
			);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts_num' ) ); ?>"><?php _e( 'Number of Sermons to Show', 'wp-sunday' ); ?>:</label>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_title' ) ); ?>"><?php _e( 'Show Sermon Title', 'wp-sunday' ); ?></label>
		</p>
